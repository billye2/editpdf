...
- **Moving text** — paragraphs can be dragged to a new position. Moves are translate-only: the original text operators are re-anchored in place (never re-encoded), so kerning, justification, fonts, and paint order survive the move exactly.
